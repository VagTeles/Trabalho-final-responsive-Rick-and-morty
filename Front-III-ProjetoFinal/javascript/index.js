...
function displayCharacters(characters) {
	...
	charactersToDisplay.forEach(function (character) {
		...
		status.innerHTML = `<span class="circle ${getStatusColor(
			character.status
		...
		.then(
			axios.spread(function (
				...
				document.getElementById(
					"characterCount"
				).textContent = `PERSONAGENS: ${characterCount}`;
				document.getElementById(
					"locationCount"
				).textContent = `LOCALIZAÇÕES: ${locationCount}`;
				document.getElementById(
					"episodeCount"
				).textContent = `EPISÓDIOS: ${episodeCount}`;
			})
		)
		.catch(function (error) {
			console.log(error);
